USE [casper]
GO
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[masterh]') AND type in (N'U'))
    DROP TABLE masterh
GO
--銀行借款編號
CREATE TABLE masterh(
...
[slipnoz]  [nvarchar](20)        NULL,--取得傳票
[slipno2e] [nvarchar](20)        NULL,--結清傳票
[slipnoe]  [nvarchar](20)        NULL,--結清傳票
...
constraint pk_masterh_num  primary key ( num )  
)
GO 
CREATE INDEX in_masterh_depano ON masterh ( depano )
CREATE INDEX in_masterh_datec  ON masterh ( datec  )
CREATE INDEX in_masterh_deplno ON masterh ( deplno )
GO
use chjer
go
IF EXISTS(SELECT NAME FROM sysobjects WHERE NAME ='masterh')
   DROP VIEW masterh
GO
CREATE VIEW masterh as select * from casper.dbo.masterh
go
